...
Noted: `src/api_splade.rs:102` — `expect("len==1 guarantees one key")` in a request handler. Low probability (only reachable if `splades.len()==1` and `keys().next()==None`, impossible by HashMap contract), but violates CLAUDE.md "panic/expect only at startup" rule. Out-of-scope for BUG-004; track as separate hardening followup.
